...
export default class HttpServer {
     ...
     */
    async findUserClassFilesInAppDir(AppDirPath, filePattern) {
        const AppsDirectories = await fs.readdir(AppDirPath, {
            withFileTypes: true,
        });
        const userClassFileDescriptions = [];
            ...
            if (appDirectory.isDirectory()) {
                ...
                for (const appDirectoryEntry of appDirectoryEntries) {
                    ...
                    if (appDirectoryEntry.name === "http") {
                        await this.findUsereClassFilesInDirectory(appDirectoryEntryPath, filePattern, userClassFileDescriptions, appName);
                    }
                }
            }
        }
     ...
     * @private
     */
    async findUsereClassFilesInDirectory(targetDirectory, filePattern, userClassFileDescriptions, appName) {
        ...
        for (const httpFile of httpFiles) {
            ...
            if (httpFile.isDirectory()) {
                await this.findUsereClassFilesInDirectory(httpFilePath, filePattern, userClassFileDescriptions, appName);
            }
     ...
     */
    logIdSchemaMap(userClassFileDescriptions) {
